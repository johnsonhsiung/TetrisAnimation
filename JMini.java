...
/**
 * A tetromini in the shape of a J.
 *
 * @author johnsonhsiung
 *
 */
public class JMini extends Tetromini
{
	/**
	 * Makes an object of JMini starting at a random location on the first row of
	 * the grid.
	 */
	public JMini()
	{
	...
	/**
	 * Draws the JMini based on its top left corner.
	 *
	...
	public void draw(Graphics2D g2)
	{
		...
		} else if (rotationNumber == 1)
		{
			...
			rect[3] = new Rectangle(topLeftX, topLeftY + Grid.BLOCK_LENGTH * 2, Grid.BLOCK_LENGTH, Grid.BLOCK_LENGTH);
		} else if (rotationNumber == 2)
		{
			...
			rect[2] = new Rectangle(topLeftX + Grid.BLOCK_LENGTH * 2, topLeftY + Grid.BLOCK_LENGTH, Grid.BLOCK_LENGTH,
					Grid.BLOCK_LENGTH);
			rect[3] = new Rectangle(topLeftX + Grid.BLOCK_LENGTH * 2, topLeftY + 2 * Grid.BLOCK_LENGTH,
	 ...
	 *
	 * @return JMini's are always BLUE, so it returns BLUE.
	 */
	@Override
