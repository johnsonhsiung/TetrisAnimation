...
/**
 * A tetromini in the shape of an T.
 *
 * @author johnsonhsiung
 *
 */
public class TMini extends Tetromini
{
	/**
	 * Makes an object of TMini starting at a random location on the first row of
	 * the grid.
	 */
	...
	/**
	 * Draws the TMini based on its top left corner.
	 *
	...
	public void draw(Graphics2D g2)
	{
		...
		} else if (rotationNumber == 1)
		{
			rect[0] = new Rectangle(topLeftX + Grid.BLOCK_LENGTH, topLeftY + Grid.BLOCK_LENGTH, Grid.BLOCK_LENGTH,
					Grid.BLOCK_LENGTH);
			rect[1] = new Rectangle(topLeftX + 2 * Grid.BLOCK_LENGTH, topLeftY, Grid.BLOCK_LENGTH, Grid.BLOCK_LENGTH);
			rect[2] = new Rectangle(topLeftX + 2 * Grid.BLOCK_LENGTH, topLeftY + Grid.BLOCK_LENGTH, Grid.BLOCK_LENGTH,
					Grid.BLOCK_LENGTH);
			rect[3] = new Rectangle(topLeftX + 2 * Grid.BLOCK_LENGTH, topLeftY + 2 * Grid.BLOCK_LENGTH,
					Grid.BLOCK_LENGTH, Grid.BLOCK_LENGTH);
		} else if (rotationNumber == 2)
		{
			rect[0] = new Rectangle(topLeftX + Grid.BLOCK_LENGTH, topLeftY + Grid.BLOCK_LENGTH, Grid.BLOCK_LENGTH,
					Grid.BLOCK_LENGTH);
			rect[1] = new Rectangle(topLeftX, topLeftY + 2 * Grid.BLOCK_LENGTH, Grid.BLOCK_LENGTH, Grid.BLOCK_LENGTH);
	 ...
	 *
	 * @return TMini's are always MAGENTA, so it returns MAGENTA.
	 */
